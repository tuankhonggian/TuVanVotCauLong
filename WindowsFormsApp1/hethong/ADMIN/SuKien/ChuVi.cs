...
{
    public partial class ChuVi : Form
    {
        ...
        private void ChuVi_Load(object sender, EventArgs e)
        {// Câu lệnh SQL để lấy dữ liệu từ bảng TapLuat
            string query = "SELECT ChuViID, NDchuvi FROM [CHUVI]"; // Chỉ lấy những cột cần thiết
        ...
        private void AddColumnsToDataGridView()
        {
            ...
            if (dataGridView_DanhSach.Columns.Count == 0)
            {
                ...
                colLuatID.Name = "ChuViID";
                colLuatID.HeaderText = "Mã Mục Đích";
                colLuatID.Width = 50;
                colLuatID.DataPropertyName = "ChuViID";  // Đảm bảo này khớp với tên cột trong DataTable
                dataGridView_DanhSach.Columns.Add(colLuatID);

                // Thêm cột "NoiDung"
                DataGridViewTextBoxColumn colNoiDung = new DataGridViewTextBoxColumn();
                colNoiDung.Name = "NDchuvi";
                colNoiDung.HeaderText = "Nội Dung";
                colNoiDung.Width = 250;
                colNoiDung.DataPropertyName = "NDchuvi"; // Đảm bảo này khớp với tên cột trong DataTable
        ...
        private void dataGridView_DanhSach_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (row.Cells["ChuViID"].Value != null && row.Cells["NDchuvi"].Value != null)
                {
                    // Hiển thị giá trị vào các TextBox
                    guna2TextBox_IDSuKien.Text = row.Cells["ChuViID"].Value.ToString();
                    guna2TextBox_NDSuKien.Text = row.Cells["NDchuvi"].Value.ToString();
        ...
        private void guna2Button_Them_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [CHUVI] WHERE ChuViID = '{mucDichID}' OR NDchuvi = N'{ndMucDich}'";
            ...
            try
            {
                ...
                string insertQuery = $"INSERT INTO [CHUVI] (ChuViID, NDchuvi) VALUES ('{mucDichID}', N'{ndMucDich}')";
        ...
        private void guna2Button_SuaLuat_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [CHUVI] WHERE NDchuvi = N'{ndMucDich}' AND ChuViID != '{mucDichID}'";
            ...
            try
            {
                ...
                string updateQuery = $"UPDATE [CHUVI] SET NDchuvi = N'{ndMucDich}' WHERE ChuViID = '{mucDichID}'";
        ...
        private void guna2Button_XoaLuat_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                string query = $"DELETE FROM [CHUVI] WHERE ChuViID = '{luatID}'";
        ...
        private void TimKiem()
        {
            ...
            string query = "SELECT ChuViID, NDchuvi FROM [CHUVI]";
            ...
            dv.RowFilter = string.Format("ChuViID LIKE '%{0}%' OR NDchuvi LIKE '%{0}%'", searchKeyword); // Lọc theo LuatID và NoiDung
