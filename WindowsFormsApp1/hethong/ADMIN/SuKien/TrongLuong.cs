...
{
    public partial class TrongLuong : Form
    {
        ...
        private void TrongLuong_Load(object sender, EventArgs e)
        {
            // Câu lệnh SQL để lấy dữ liệu từ bảng TapLuat
            string query = "SELECT TrongLuongID, NDtrongluong FROM [TRONGLUONG]"; // Chỉ lấy những cột cần thiết
        ...
        private void AddColumnsToDataGridView()
        {
            ...
            if (dataGridView_DanhSach.Columns.Count == 0)
            {
                ...
                colLuatID.Name = "TrongLuongID";
                colLuatID.HeaderText = "Mã Mục Đích";
                colLuatID.Width = 50;
                colLuatID.DataPropertyName = "TrongLuongID";  // Đảm bảo này khớp với tên cột trong DataTable
                dataGridView_DanhSach.Columns.Add(colLuatID);

                // Thêm cột "NoiDung"
                DataGridViewTextBoxColumn colNoiDung = new DataGridViewTextBoxColumn();
                colNoiDung.Name = "NDtrongluong";
                colNoiDung.HeaderText = "Nội Dung";
                colNoiDung.Width = 250;
                colNoiDung.DataPropertyName = "NDtrongluong"; // Đảm bảo này khớp với tên cột trong DataTable
        ...
        private void dataGridView_DanhSach_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (row.Cells["TrongLuongID"].Value != null && row.Cells["NDtrongluong"].Value != null)
                {
                    // Hiển thị giá trị vào các TextBox
                    guna2TextBox_IDSuKien.Text = row.Cells["TrongLuongID"].Value.ToString();
                    guna2TextBox_NDSuKien.Text = row.Cells["NDtrongluong"].Value.ToString();
        ...
        private void guna2Button_Them_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [TRONGLUONG] WHERE TrongLuongID = '{mucDichID}' OR NDtrongluong = N'{ndMucDich}'";
            ...
            try
            {
                ...
                string insertQuery = $"INSERT INTO [TRONGLUONG] (TrongLuongID, NDtrongluong) VALUES ('{mucDichID}', N'{ndMucDich}')";
        ...
        private void guna2Button_SuaLuat_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [TRONGLUONG] WHERE NDtrongluong = N'{ndMucDich}' AND TrongLuongID != '{mucDichID}'";
            ...
            try
            {
                ...
                string updateQuery = $"UPDATE [TRONGLUONG] SET NDtrongluong = N'{ndMucDich}' WHERE TrongLuongID = '{mucDichID}'";
        ...
        private void guna2Button_XoaLuat_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                string query = $"DELETE FROM [TRONGLUONG] WHERE TrongLuongID = '{luatID}'";
        ...
        private void TimKiem()
        {
            ...
            string query = "SELECT TrongLuongID, NDtrongluong FROM [TRONGLUONG]";
            ...
            dv.RowFilter = string.Format("TrongLuongID LIKE '%{0}%' OR NDtrongluong LIKE '%{0}%'", searchKeyword); // Lọc theo LuatID và NoiDung
