...
{
    public partial class MatVot : Form
    {
        ...
        private void MatVot_Load(object sender, EventArgs e)
        {
            // Câu lệnh SQL để lấy dữ liệu từ bảng TapLuat
            string query = "SELECT MatVotID, NDmatvot FROM [MATVOT]"; // Chỉ lấy những cột cần thiết
        ...
        private void AddColumnsToDataGridView()
        {
            ...
            if (dataGridView_DanhSach.Columns.Count == 0)
            {
                ...
                colLuatID.Name = "MatVotID";
                colLuatID.HeaderText = "Mã Mục Đích";
                colLuatID.Width = 50;
                colLuatID.DataPropertyName = "MatVotID";  // Đảm bảo này khớp với tên cột trong DataTable
                dataGridView_DanhSach.Columns.Add(colLuatID);

                // Thêm cột "NoiDung"
                DataGridViewTextBoxColumn colNoiDung = new DataGridViewTextBoxColumn();
                colNoiDung.Name = "NDmatvot";
                colNoiDung.HeaderText = "Nội Dung";
                colNoiDung.Width = 250;
                colNoiDung.DataPropertyName = "NDmatvot"; // Đảm bảo này khớp với tên cột trong DataTable
        ...
        private void dataGridView_DanhSach_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (row.Cells["MatVotID"].Value != null && row.Cells["NDmatvot"].Value != null)
                {
                    // Hiển thị giá trị vào các TextBox
                    guna2TextBox_IDSuKien.Text = row.Cells["MatVotID"].Value.ToString();
                    guna2TextBox_NDSuKien.Text = row.Cells["NDmatvot"].Value.ToString();
        ...
        private void guna2Button_Them_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [MATVOT] WHERE MatVotID = '{mucDichID}' OR NDmatvot = N'{ndMucDich}'";
            ...
            try
            {
                ...
                string insertQuery = $"INSERT INTO [MATVOT] (MatVotID, NDmatvot) VALUES ('{mucDichID}', N'{ndMucDich}')";
        ...
        private void guna2Button_SuaLuat_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [MATVOT] WHERE NDmatvot = N'{ndMucDich}' AND MatVotID != '{mucDichID}'";
            ...
            try
            {
                ...
                string updateQuery = $"UPDATE [MATVOT] SET NDmatvot = N'{ndMucDich}' WHERE MatVotID = '{mucDichID}'";
        ...
        private void guna2Button_XoaLuat_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                string query = $"DELETE FROM [MATVOT] WHERE MatVotID = '{luatID}'";
        ...
        private void TimKiem()
        {
            ...
            string query = "SELECT MatVotID, NDmatvot FROM [MATVOT]";
            ...
            dv.RowFilter = string.Format("MatVotID LIKE '%{0}%' OR NDmatvot LIKE '%{0}%'", searchKeyword); // Lọc theo LuatID và NoiDung
