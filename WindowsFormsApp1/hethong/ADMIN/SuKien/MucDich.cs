...
{
    public partial class MucDich : Form
    {
        ...
        private void MucDich_Load(object sender, EventArgs e)
        {
            // Câu lệnh SQL để lấy dữ liệu từ bảng TapLuat
            string query = "SELECT MucDichID, NDmucdich FROM [MUCDICH]"; // Chỉ lấy những cột cần thiết
        ...
        private void AddColumnsToDataGridView()
        {
            ...
            if (dataGridView_DanhSach.Columns.Count == 0)
            {
                ...
                colLuatID.Name = "MucDichID";
                colLuatID.HeaderText = "Mã Mục Đích";
                colLuatID.Width = 50;
                colLuatID.DataPropertyName = "MucDichID";  // Đảm bảo này khớp với tên cột trong DataTable
                dataGridView_DanhSach.Columns.Add(colLuatID);

                // Thêm cột "NoiDung"
                DataGridViewTextBoxColumn colNoiDung = new DataGridViewTextBoxColumn();
                colNoiDung.Name = "NDmucdich";
                colNoiDung.HeaderText = "Nội Dung";
                colNoiDung.Width = 250;
                colNoiDung.DataPropertyName = "NDmucdich"; // Đảm bảo này khớp với tên cột trong DataTable
        ...
        private void dataGridView_DanhSach_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (row.Cells["MucDichID"].Value != null && row.Cells["NDmucdich"].Value != null)
                {
                    // Hiển thị giá trị vào các TextBox
                    guna2TextBox_IDSuKien.Text = row.Cells["MucDichID"].Value.ToString();
                    guna2TextBox_NDSuKien.Text = row.Cells["NDmucdich"].Value.ToString();
        ...
        private void guna2Button_SuaLuat_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [MUCDICH] WHERE NDmucdich = N'{ndMucDich}' AND MucDichID != '{mucDichID}'";
            ...
            try
            {
                ...
                string updateQuery = $"UPDATE [MUCDICH] SET NDmucdich = N'{ndMucDich}' WHERE MucDichID = '{mucDichID}'";

                // Thực thi câu lệnh SQL
                ketnoi.Thucthi(updateQuery);

                // Thông báo sửa thành công
                MessageBox.Show("Cập nhật mục đích thành công!");

                // Làm mới lại danh sách
                MucDich_Load(sender, e);
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Có lỗi xảy ra khi cập nhật mục đích: " + ex.Message);
            }
        }
        ...
        private void guna2Button_Them_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [MUCDICH] WHERE MucDichID = '{mucDichID}' OR NDmucdich = N'{ndMucDich}'";
            ...
            try
            {
                ...
                if (exists > 0)
                {
                    MessageBox.Show("Mục đích đã tồn tại, vui lòng nhập thông tin khác.");
                    return;
                }

                // Câu lệnh SQL để thêm dữ liệu vào bảng MUCDICH
                string insertQuery = $"INSERT INTO [MUCDICH] (MucDichID, NDmucdich) VALUES ('{mucDichID}', N'{ndMucDich}')";

                // Thực thi câu lệnh SQL
                ketnoi.Thucthi(insertQuery);

                // Thông báo thêm thành công
                MessageBox.Show("Thêm mục đích thành công!");
            ...
            {
                // Thông báo lỗi nếu có vấn đề
                MessageBox.Show("Có lỗi xảy ra khi thêm mục đích: " + ex.Message);
            }
        }
        ...
        private void guna2Button_XoaLuat_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                string query = $"DELETE FROM [MUCDICH] WHERE MucDichID = '{luatID}'";
        ...
        private void TimKiem()
        {
            ...
            string query = "SELECT MucDichID, NDmucdich FROM [MUCDICH]";
            ...
            dv.RowFilter = string.Format("MucDichID LIKE '%{0}%' OR NDmucdich LIKE '%{0}%'", searchKeyword); // Lọc theo LuatID và NoiDung
