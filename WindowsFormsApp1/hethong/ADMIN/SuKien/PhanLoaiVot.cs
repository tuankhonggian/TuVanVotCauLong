...
{
    public partial class PhanLoaiVot : Form
    {
        ...
        private void PhanLoaiVot_Load(object sender, EventArgs e)
        {
            // Câu lệnh SQL để lấy dữ liệu từ bảng TapLuat
            string query = "SELECT PhanLoaiVotID, NDphanloaivot FROM [PHANLOAIVOT]"; // Chỉ lấy những cột cần thiết
        ...
        private void AddColumnsToDataGridView()
        {
            ...
            if (dataGridView_DanhSach.Columns.Count == 0)
            {
                ...
                colLuatID.Name = "PhanLoaiVotID";
                colLuatID.HeaderText = "Mã Mục Đích";
                colLuatID.Width = 50;
                colLuatID.DataPropertyName = "PhanLoaiVotID";  // Đảm bảo này khớp với tên cột trong DataTable
                dataGridView_DanhSach.Columns.Add(colLuatID);

                // Thêm cột "NoiDung"
                DataGridViewTextBoxColumn colNoiDung = new DataGridViewTextBoxColumn();
                colNoiDung.Name = "NDphanloaivot";
                colNoiDung.HeaderText = "Nội Dung";
                colNoiDung.Width = 250;
                colNoiDung.DataPropertyName = "NDphanloaivot"; // Đảm bảo này khớp với tên cột trong DataTable
        ...
        private void dataGridView_DanhSach_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (row.Cells["PhanLoaiVotID"].Value != null && row.Cells["NDphanloaivot"].Value != null)
                {
                    // Hiển thị giá trị vào các TextBox
                    guna2TextBox_IDSuKien.Text = row.Cells["PhanLoaiVotID"].Value.ToString();
                    guna2TextBox_NDSuKien.Text = row.Cells["NDphanloaivot"].Value.ToString();
        ...
        private void guna2Button_Them_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [PHANLOAIVOT] WHERE PhanLoaiVotID = '{mucDichID}' OR NDphanloaivot = N'{ndMucDich}'";
            ...
            try
            {
                ...
                string insertQuery = $"INSERT INTO [PHANLOAIVOT] (PhanLoaiVotID, NDphanloaivot) VALUES ('{mucDichID}', N'{ndMucDich}')";
        ...
        private void guna2Button_SuaLuat_Click(object sender, EventArgs e)
        {
            ...
            string checkQuery = $"SELECT COUNT(*) FROM [PHANLOAIVOT] WHERE NDphanloaivot = N'{ndMucDich}' AND PhanLoaiVotID != '{mucDichID}'";
            ...
            try
            {
                ...
                string updateQuery = $"UPDATE [PHANLOAIVOT] SET NDphanloaivot = N'{ndMucDich}' WHERE PhanLoaiVotID = '{mucDichID}'";
        ...
        private void guna2Button_XoaLuat_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                string query = $"DELETE FROM [PHANLOAIVOT] WHERE PhanLoaiVotID = '{luatID}'";
        ...
        private void TimKiem()
        {
            ...
            string query = "SELECT PhanLoaiVotID, NDphanloaivot FROM [PHANLOAIVOT]";
            ...
            dv.RowFilter = string.Format("PhanLoaiVotID LIKE '%{0}%' OR NDphanloaivot LIKE '%{0}%'", searchKeyword); // Lọc theo LuatID và NoiDung
